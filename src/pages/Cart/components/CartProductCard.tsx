import React from "react";
import {
  minusProduct,
  plusProduct,
  removeProduct,
} from "../../../redux/cart/slice";
import { CartProduct } from "../../../redux/cart/types";
import { useAppDispatch } from "../../../redux/store";

export const CartProductCard: React.FC<CartProduct> = ({
  id,
  title,
  subtitle,
  imageUrl,
  color,
  size,
  price,
  value,
}) => {
  const dispatch = useAppDispatch();
  return (
    <div className="cartProductCard">
      <img
        src={require(`../../.././assets/img/products/${imageUrl}.jpg`)}
        alt="img"
      />
      <div className="cartProductCard__text">
        <h3>
          {title} <span> {subtitle}</span>
        </h3>
        <p>
          {color},&nbsp;
          {size}
        </p>
      </div>
      <div className="cartProductCard__counter">
        <button
          onClick={() => dispatch(minusProduct({ id, color, size, value }))}
          disabled={value === 1}
        >
          -
        </button>
        {value}
        <button
          onClick={() => dispatch(plusProduct({ id, color, size, value }))}
        >
          +
        </button>
      </div>
      <div className="cartProductCart__price">${price * value}</div>
      <button onClick={() => dispatch(removeProduct({ id, color, size }))}>
        <svg
          width="25px"
          height="28px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M16.9498 8.46447C17.3404 8.07394 17.3404 7.44078 16.9498 7.05025C16.5593 6.65973 15.9261 6.65973 15.5356 7.05025L12.0001 10.5858L8.46455 7.05025C8.07402 6.65973 7.44086 6.65973 7.05033 7.05025C6.65981 7.44078 6.65981 8.07394 7.05033 8.46447L10.5859 12L7.05033 15.5355C6.65981 15.9261 6.65981 16.5592 7.05033 16.9497C7.44086 17.3403 8.07402 17.3403 8.46455 16.9497L12.0001 13.4142L15.5356 16.9497C15.9261 17.3403 16.5593 17.3403 16.9498 16.9497C17.3404 16.5592 17.3404 15.9261 16.9498 15.5355L13.4143 12L16.9498 8.46447Z"
            fill="#000000"
          />
        </svg>
      </button>
    </div>
  );
};
